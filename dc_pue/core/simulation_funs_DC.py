...
def Cooling_Tower(
    ...
    Latent_heat_vaporization = (
        lambda T_water: -0.0013 * (T_water) ** (2) - 2.3097 * (T_water) + 2500.5
    )
...
def PUE_WUE_AE_Chiller(w):
    ...
    # functions````````
    Fan_Power = (
        lambda mass_flow_rate, air_density, Fan_Pressure, Fan_e: mass_flow_rate
        / air_density
        * Fan_Pressure
        / Fan_e
        / 1000
    ...
    Pump_Power = (
        lambda mass_flow_rate, Pump_Pressure, Pump_e, liquid_density: Pump_Pressure
        * mass_flow_rate
        / (1000 * Pump_e * liquid_density)
    )
    # ``````````````````
